...
impl<'a> TemplateManager<'a> {
    pub fn inject_template_files(
        ...
        let is_empty = is_dir_empty(output).map_err(|e| IoError::ReadDir(e, output.clone()))?;
        ...
        if should_copy {
            template.inject(&output, variant.as_deref())?;
        }
...
//     use crate::{
//         cli::commands::template::{
//             test_utils::test_utils::create_test_manager, TemplateError, DEFAULT_FOLDER,
//         },
//         utils::input::Input,
//     };
