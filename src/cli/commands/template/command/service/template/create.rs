...
use crate::{
    cli::commands::template::{
        command::{service::TemplateService, sub_commands::TemplateSubcommands, utils::Template},
...
impl<'a> TemplateService<'a> {
    pub fn create_template(&self, template: &str) -> Result<Template, TemplateError> {
        let template = slug(template).map_err(TemplateError::Validation)?;
