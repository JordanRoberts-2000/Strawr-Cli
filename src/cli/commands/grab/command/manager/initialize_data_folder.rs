...
use crate::{cli::commands::grab::GrabError, state::AppContext};

use super::GrabManager;

impl GrabManager {
    pub fn initialize_data_folder(&mut self, ctx: &AppContext) -> Result<(), GrabError> {
        ...
        if !self.list_file_path.exists() {
            fs::write(&self.list_file_path, "").map_err(|e| GrabError::Io {
                source: e,
